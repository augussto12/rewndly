...
namespace MovieSys.IntegrationTests;
...
public sealed class CriticalFlowsIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task Refresh_token_reuse_is_rejected()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/auth/register", new
        {
            ...
            email = $"itreuse{stamp}@moviesys.local",
            password = "User123!",
        ...
        var oldRefreshCookie = response.Headers.GetValues("Set-Cookie")
            .Single(header => header.StartsWith("moviesys_refresh_token=", StringComparison.Ordinal))
            .Split(';')[0];
    ...
    private static string GetConnectionString()
    {
        return Environment.GetEnvironmentVariable("ConnectionStrings__DefaultConnection")
            ...
            ?? throw new InvalidOperationException("Integration tests require ConnectionStrings__DefaultConnection or MOVIESYS_CONNECTION_STRING.");
    }
