    ...
    ILogger<TmdbClient> logger) : IPublicMediaService
{
    ...
    public async Task<PublicHomeResponse> GetHomeAsync(CancellationToken cancellationToken)
    {
        ...
        return new PublicHomeResponse(trendingMovies, popularMovies, upcomingMovies, trendingSeries, popularSeries);
    }
        ...
        return GetCachedAsync<IReadOnlyList<MediaSummaryResponse>>(
            ...
            async () =>
            {
                ...
                var response = await GetAsync<TmdbSearchResponseDto<TmdbMovieDto>>(
                    $"search/movie?query={Uri.EscapeDataString(normalized)}&include_adult=false&language=es-AR&page=1",
                    cancellationToken);
    ...
    {
        return GetMovieListAsync("tmdb:movies:trending", "trending/movie/week?language=es-AR", TrendingTtl, cancellationToken);
    }

    public Task<IReadOnlyList<MediaSummaryResponse>> GetPopularMoviesAsync(CancellationToken cancellationToken)
    {
        return GetMovieListAsync("tmdb:movies:popular", "movie/popular?language=es-AR&page=1", PopularTtl, cancellationToken);
    }

    public Task<IReadOnlyList<MediaSummaryResponse>> GetUpcomingMoviesAsync(CancellationToken cancellationToken)
    {
        return GetMovieListAsync("tmdb:movies:upcoming", "movie/upcoming?language=es-AR&page=1", PopularTtl, cancellationToken);
    }
    ...
    public Task<MovieDetailsResponse?> GetMovieDetailsAsync(int tmdbId, CancellationToken cancellationToken)
    {
        return GetCachedAsync<MovieDetailsResponse?>(
            ...
            async () =>
            {
                ...
                try
                {
                    movie = await GetAsync<TmdbMovieDto>($"movie/{tmdbId}?language=es-AR", cancellationToken);
                }
        ...
        return GetCachedAsync<IReadOnlyList<MediaSummaryResponse>>(
            ...
            async () =>
            {
                ...
                var response = await GetAsync<TmdbSearchResponseDto<TmdbSeriesDto>>(
                    $"search/tv?query={Uri.EscapeDataString(normalized)}&include_adult=false&language=es-AR&page=1",
                    cancellationToken);
    ...
    {
        return GetSeriesListAsync("tmdb:series:trending", "trending/tv/week?language=es-AR", TrendingTtl, cancellationToken);
    }

    public Task<IReadOnlyList<MediaSummaryResponse>> GetPopularSeriesAsync(CancellationToken cancellationToken)
    {
        return GetSeriesListAsync("tmdb:series:popular", "tv/popular?language=es-AR&page=1", PopularTtl, cancellationToken);
    }
    ...
    public Task<SeriesDetailsResponse?> GetSeriesDetailsAsync(int tmdbId, CancellationToken cancellationToken)
    {
        return GetCachedAsync<SeriesDetailsResponse?>(
            ...
            async () =>
            {
                ...
                try
                {
                    series = await GetAsync<TmdbSeriesDto>($"tv/{tmdbId}?language=es-AR", cancellationToken);
                }
                catch (TmdbNotFoundException)
                {
                    return null;
    ...
    public Task<IReadOnlyList<GenreResponse>> GetMovieGenresAsync(CancellationToken cancellationToken)
    {
        return GetCachedAsync<IReadOnlyList<GenreResponse>>(
            ...
            async () =>
            {
                var response = await GetAsync<TmdbGenresResponseDto>("genre/movie/list?language=es-AR", cancellationToken);
    ...
    public Task<IReadOnlyList<GenreResponse>> GetSeriesGenresAsync(CancellationToken cancellationToken)
    {
        return GetCachedAsync<IReadOnlyList<GenreResponse>>(
            ...
            async () =>
            {
                var response = await GetAsync<TmdbGenresResponseDto>("genre/tv/list?language=es-AR", cancellationToken);
                return response.Genres.Select(genre => new GenreResponse(genre.Id, genre.Name)).ToList();
            });
    }
