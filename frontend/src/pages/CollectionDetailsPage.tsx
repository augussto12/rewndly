    ...
    <PublicLayout>
      ...
      {isError || !Number.isFinite(collectionId) ? (
        <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6">
          <ErrorState title="No pudimos cargar la coleccion" />
        </main>
      ) : null}
      ...
      {!isLoading && !isError && data ? (
        <main>
            ...
            <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-[14rem_1fr] md:py-16">
              <div className="w-44 md:w-full">
                <div className="aspect-[2/3] overflow-hidden rounded-[var(--radius-md)] border border-white/[0.12] bg-[var(--color-surface-elevated)] shadow-[var(--shadow-poster)]">
                  {data.posterUrl ? <img src={data.posterUrl} alt={data.name} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-sm text-[var(--color-text-secondary)]">Sin poster</div>}
                </div>
              </div>
              <div className="self-end">
                <p className="kicker">Coleccion</p>
          ...
          <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
            ...
            <h2 className="mt-2 text-2xl font-semibold">Peliculas de la saga</h2>
            <div className="mt-6">
              {data.parts.length > 0 ? <MediaGrid items={data.parts} /> : <EmptyState title="Sin partes disponibles" message="TMDB no devolvio peliculas para esta coleccion." />}
            </div>
          </section>
        </main>
      ) : null}
