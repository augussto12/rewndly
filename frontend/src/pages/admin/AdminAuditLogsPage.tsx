        ...
        {data ? (
          <>
            <AdminTable<AdminAuditLog>
              items={data.items}
              columns={[
                { header: 'Acción', cell: (log) => <AdminEventRow title={log.action} meta={`${log.targetType} ${log.targetId ?? ''}`} /> },
...
function Header({ title }: { title: string }) {
  return <header className="mb-8"><p className="kicker">Auditoría</p><h1 className="mt-3 text-4xl font-semibold">{title}</h1></header>
}
