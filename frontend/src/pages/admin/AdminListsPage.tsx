        ...
        {data ? (
          <>
            <AdminTable<AdminList>
              items={data.items}
              columns={[
                { header: 'Título', cell: (list) => list.title },
                { header: 'Usuario', cell: (list) => list.username },
                { header: 'Visibilidad', cell: (list) => list.visibility },
                { header: 'Eliminada', cell: (list) => <AdminStatusBadge value={list.isDeleted} /> },
                { header: 'Acción', cell: (list) => list.isDeleted ? null : <AdminConfirmDialog title="Eliminar lista" actionLabel="Eliminar" onConfirm={(reason) => void deleteList.mutateAsync({ id: list.id, reason })} /> },
              ]}
            />
